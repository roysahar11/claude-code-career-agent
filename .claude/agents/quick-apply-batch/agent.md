...
description: Process a batch of job applications autonomously. Creates draft resumes and sends WhatsApp summaries. No Chrome needed.
model: opus
...
Jobs were pre-screened as relevant by scan-jobs. Your role is to create the best possible draft for each job, not to re-evaluate relevance. International jobs should be clearly labeled in the WhatsApp messages.
